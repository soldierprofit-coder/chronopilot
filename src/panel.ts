...
const CHRONO_ABILITIES: Array<[AbilityId, string]> = [
  ['arcane_intellect', 'Aether Insight'],
  ['ice_floes', 'Ice Floes'],
...
const CONTEXTS: Array<[CombatContextMode, string, number]> = [
...
export function mountChronoPilotPanel(controller: ChronoPilotController): MountedChronoPilotPanel {
  injectStyle();
  ...
  const tabs = root.querySelector('.cp-tabs') as HTMLElement;
  const views = root.querySelector('.cp-views') as HTMLElement;
  const persist = (): void => saveSettings(controller.settings);
  ...
  rotationSelect.addEventListener('change', () => {
    controller.settings.assistProfile = rotationSelect.value as typeof controller.settings.assistProfile;
    persist();
  });
  ...
  profileSelect.addEventListener('change', () => {
    controller.settings.mode = profileSelect.value as typeof controller.settings.mode;
    persist();
  });
  ...
  const profileFields = new Map<CombatContextMode, HTMLElement>();
  for (const [context, label, maxAllies] of CONTEXTS) {
    ...
    field.append(
      range('Temporal Mend', profile.mendHpPct * 100, 30, 95, percent, (value) => { profile.mendHpPct = value / 100; persist(); }),
      range('Temporal Barrier', profile.barrierHpPct * 100, 30, 100, percent, (value) => { profile.barrierHpPct = value / 100; persist(); }),
      range('Emergency heal', profile.emergencyHpPct * 100, 15, 80, percent, (value) => { profile.emergencyHpPct = value / 100; persist(); }),
      range('Cascade below', profile.cascadeHpPct * 100, 50, 100, percent, (value) => { profile.cascadeHpPct = value / 100; persist(); }),
      range('Cascade ally count', profile.cascadeCount, 1, maxAllies, String, (value) => { profile.cascadeCount = value; persist(); }),
      range('Mass Barrier below', profile.massBarrierHpPct * 100, 40, 95, percent, (value) => { profile.massBarrierHpPct = value / 100; persist(); }),
      range('Mass Barrier ally count', profile.massBarrierCount, 1, maxAllies, String, (value) => { profile.massBarrierCount = value; persist(); }),
      range('Rewind recent loss', profile.rewindLossPct * 100, 5, 60, percent, (value) => { profile.rewindLossPct = value / 100; persist(); }),
      range('Rewind ally count', profile.rewindCount, 1, maxAllies, String, (value) => { profile.rewindCount = value; persist(); }),
      range('Conserve mana below', profile.conserveManaPct * 100, 0, 80, percent, (value) => { profile.conserveManaPct = value / 100; persist(); }),
      range('Stop damage below', profile.stopDamageManaPct * 100, 0, 60, percent, (value) => { profile.stopDamageManaPct = value / 100; persist(); }),
    );
  ...
  manaField.append(
    range('Aetherwell mana', controller.settings.thresholds.aetherwellManaPct * 100, 10, 70, percent, (value) => { controller.settings.thresholds.aetherwellManaPct = value / 100; persist(); }),
    checkbox('Smart Power Echo rescue', controller.settings.thresholds.smartPowerEcho, (value) => { controller.settings.thresholds.smartPowerEcho = value; persist(); }),
    checkbox('Smart Perfect Moment healing', controller.settings.thresholds.smartPerfectMoment, (value) => { controller.settings.thresholds.smartPerfectMoment = value; persist(); }),
    checkbox('Smart Surge charges', controller.settings.thresholds.smartSurgeCharges, (value) => { controller.settings.thresholds.smartSurgeCharges = value; persist(); }),
    range('Maximum Surge charges', controller.settings.thresholds.maxSurgeCharges, 1, 4, (value) => String(value), (value) => { controller.settings.thresholds.maxSurgeCharges = value; persist(); }),
    range('Minimum Surge charges', controller.settings.thresholds.lowManaMaxSurgeCharges, 1, 4, (value) => String(value), (value) => { controller.settings.thresholds.lowManaMaxSurgeCharges = value; persist(); }),
    range('Aetherburst enemies', controller.settings.thresholds.aoeEnemyCount, 2, 6, (value) => String(value), (value) => { controller.settings.thresholds.aoeEnemyCount = value; persist(); }),
  );
  ...
  potionField.append(
    checkbox('Use health potions', controller.settings.consumables.healthPotion, (value) => { controller.settings.consumables.healthPotion = value; persist(); }),
    range('Health potion below', controller.settings.consumables.healthPotionHpPct * 100, 10, 70, percent, (value) => { controller.settings.consumables.healthPotionHpPct = value / 100; persist(); }),
    checkbox('Use mana potions', controller.settings.consumables.manaPotion, (value) => { controller.settings.consumables.manaPotion = value; persist(); }),
    range('Mana potion below', controller.settings.consumables.manaPotionManaPct * 100, 5, 60, percent, (value) => { controller.settings.consumables.manaPotionManaPct = value / 100; persist(); }),
  );
  ...
  enemySelect.addEventListener('change', () => {
    controller.settings.targeting.enemyMode = enemySelect.value as typeof controller.settings.targeting.enemyMode;
    persist();
  });
  ...
  const refreshAssistMembers = (): void => {
  ...
  targetField.append(
    enemyRow,
    assistRow,
    range('Target range', controller.settings.targeting.maxTargetRange, 5, 30, (value) => `${value} yd`, (value) => { controller.settings.targeting.maxTargetRange = value; persist(); }),
    checkbox('Attack new enemies in Solo Questing', controller.settings.targeting.autoPull, (value) => { controller.settings.targeting.autoPull = value; persist(); }),
    checkbox('Keep Echo on tank (off = adaptive rescue)', controller.settings.targeting.keepEchoOnTank, (value) => { controller.settings.targeting.keepEchoOnTank = value; persist(); }),
    checkbox('Party and raid targets only', controller.settings.targeting.partyOnly, (value) => { controller.settings.targeting.partyOnly = value; persist(); }),
    checkbox('Natural stream targeting', controller.settings.targeting.streamTargetSelection, (value) => { controller.settings.targeting.streamTargetSelection = value; persist(); }),
  );
  ...
  safetyField.append(
    range('Manual pause', controller.settings.safety.manualOverrideMs / 100, 1, 50, (value) => `${(value / 10).toFixed(1)} sec`, (value) => { controller.settings.safety.manualOverrideMs = value * 100; persist(); }),
    checkbox('Enable PvP / Arena assist', controller.settings.pvp.enabled, (value) => { controller.settings.pvp.enabled = value; controller.settings.safety.disableInPvp = !value; persist(); }),
    range('PvP maximum Surge charges', controller.settings.pvp.maxSurgeCharges, 1, 4, String, (value) => { controller.settings.pvp.maxSurgeCharges = value; persist(); }),
    range('PvP minimum Surge charges', controller.settings.pvp.minSurgeCharges, 1, 4, String, (value) => { controller.settings.pvp.minSurgeCharges = value; persist(); }),
    range('PvP Ice Block below', controller.settings.pvp.iceBlockHpPct * 100, 10, 60, percent, (value) => { controller.settings.pvp.iceBlockHpPct = value / 100; persist(); }),
    checkbox('Blink out of roots', controller.settings.pvp.blinkOnRoot, (value) => { controller.settings.pvp.blinkOnRoot = value; persist(); }),
    range('PvP Hourglass rescue', controller.settings.pvp.hourglassHpPct * 100, 10, 50, percent, (value) => { controller.settings.pvp.hourglassHpPct = value / 100; persist(); }),
  );
  ...
  for (const [id, label, view] of sections) {
    ...
    button.addEventListener('click', () => {
      ...
      for (const candidate of root.querySelectorAll<HTMLElement>('[data-tab]')) {
        candidate.dataset.selected = String(candidate === button);
      }
      for (const candidate of root.querySelectorAll<HTMLElement>('[data-view]')) {
  ...
  const toggle = root.querySelector('.cp-toggle') as HTMLButtonElement;
  const pause = root.querySelector('.cp-pause') as HTMLButtonElement;
  toggle.addEventListener('click', () => controller.toggle());
  pause.addEventListener('click', () => controller.stop('Emergency pause pressed.'));

  const head = root.querySelector('.cp-head') as HTMLElement;
