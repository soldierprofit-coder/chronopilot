...
function abilityReady(
  ...
  return (
    settings.fireAbilities[abilityId] &&
...
function fireCluster(
  ...
  let best = {
    ...
    aoeSafe: !observation.enemies.some(
      (enemy) => !enemy.dead && protectedControl(enemy) && distance(primary, enemy) <= 8,
    ),
  };
  for (const center of eligible) {
    ...
    const aoeSafe = !observation.enemies.some(
      (enemy) => !enemy.dead && protectedControl(enemy) && distance(center, enemy) <= 8,
    );
    if (!aoeSafe) continue;
...
export function decideFire(
  ...
  if (playerInStasis) return wait('Cold Coffin is active; hold until immunity ends or you cancel it.');
  ...
  const nearbyAttackers = observation.enemies.filter(
    (candidate) => candidate.hostile && !candidate.dead && !protectedControl(candidate) && distance(player, candidate) <= 10,
  );
  if (
    pvpMode &&
    !player.castingAbility &&
  ...
  if (
    ...
    const secondary = observation.enemies
      ...
      .sort((a, b) => distance(player, a) - distance(player, b) || a.id - b.id)[0];
    if (secondary) return cast('polymorph', 15, `Control ${secondary.name} while the team is pressured.`, secondary.id);
  }
  if (
    pvpMode &&
    !player.castingAbility &&
  ...
  if (!gcdReady) return wait('Waiting for the global cooldown.');
  ...
  const breathTargets = observation.enemies.filter(
    (candidate) => candidate.hostile && !candidate.dead && !protectedControl(candidate) && distance(player, candidate) <= 12,
  ).length;
  if (
    !conservingMana &&
  ...
  if (
    abilityReady(observation, settings, 'scorch') &&
