...
function castAt(abilityId, priority, reason, enemy) {
    return {
        type: 'cast-at',
        abilityId,
        x: enemy.x,
...
function abilityReady(observation, settings, abilityId) {
    ...
    return (settings.frostAbilities[abilityId] &&
...
function frostCluster(observation, primary) {
    ...
    const primarySafe = !observation.enemies.some((enemy) => !enemy.dead && protectedControl(enemy) && distance(primary, enemy) <= 7);
    ...
    for (const center of eligible) {
        ...
        const aoeSafe = !observation.enemies.some((enemy) => !enemy.dead && protectedControl(enemy) && distance(center, enemy) <= 7);
        if (!aoeSafe)
            continue;
...
function mageFrozen(enemy, playerId) {
    return enemy.auras.some((aura) => aura.sourceId === playerId && FROZEN_CONTROL.has(aura.kind));
}
export function resolveAssistProfile(observation, settings) {
    ...
    if (observation.talentSpec === 'frost')
        return 'frost-pve';
...
export function decideFrost(observation, settings) {
    ...
    if (observation.pvp)
        return wait('Frost PvE is paused during active PvP.');
    ...
    if (settings.modules.interrupts &&
        ...
        if (caster)
            return cast('counterspell', 4, `Interrupt ${caster.name}.`, caster.id);
    }
    ...
    if (!enemy)
        return wait('No valid PvE enemy is selected or engaged in range.');
    ...
    const nearbyEnemies = observation.enemies.filter((candidate) => !candidate.dead && !protectedControl(candidate) && distance(player, candidate) <= 10);
    const frontalSafe = !observation.enemies.some((candidate) => !candidate.dead && protectedControl(candidate) && distance(player, candidate) <= 16);
    ...
    if (abilityReady(observation, settings, 'icy_veins') &&
        player.inCombat &&
    ...
    if (icicles >= 5 && abilityReady(observation, settings, 'glacial_spike')) {
        if (settings.frost.smartGlacialBurst &&
    ...
    if (abilityReady(observation, settings, 'frozen_orb') &&
    ...
    if (nearbyEnemies.length >= settings.frost.blizzardEnemyCount &&
        frontalSafe &&
