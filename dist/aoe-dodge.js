...
function pathEntersNewZone(start, end, zones) {
    ...
    for (const zone of zones) {
        if (inside(start, zone, AOE_SAFETY_MARGIN))
            continue;
        for (let step = 1; step < samples; step++) {
            ...
            if (inside(point, zone, AOE_SAFETY_MARGIN))
                return true;
        }
    }
    return false;
 ...
 */
export function planAoeDodge(player, source, continuing = false) {
    ...
    for (let index = 0; index < ANGLE_STEPS; index++) {
        ...
        for (let distance = DISTANCE_STEP; distance <= MAX_ESCAPE_DISTANCE; distance += DISTANCE_STEP) {
            ...
            if (zones.some((zone) => inside(point, zone, AOE_SAFETY_MARGIN + EXIT_CLEARANCE)))
                continue;
            if (pathEntersNewZone(player, point, zones))
                break;
            const score = distance + Math.abs(offsetIndex) * 0.015;
            if (!best || score < best.score)
                best = { ...point, angle, distance, score };
            break;
        }
    }
    if (!best)
    ...
    const blinkSafe = zones.every((zone) => !inside(blinkLanding, zone, AOE_SAFETY_MARGIN + EXIT_CLEARANCE));
