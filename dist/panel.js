...
export function mountChronoPilotPanel(controller) {
  ...
  `;
    ...
    mechanicField.append(checkbox('Dodge AoE (emergency Flickerstep)', controller.settings.safety.dodgeAoe, (value) => {
    ...
    fireField.append(checkbox('Smart Fire burst', controller.settings.fire.smartBurst, (value) => { controller.settings.fire.smartBurst = value; persist(); }), checkbox('Smart Blazing Barrier pre-shield', controller.settings.fire.smartPreShield, (value) => { controller.settings.fire.smartPreShield = value; persist(); }), checkbox('Phoenix Trance on durable targets', controller.settings.fire.phoenixTranceDurableOnly, (value) => { controller.settings.fire.phoenixTranceDurableOnly = value; persist(); }), checkbox('Meteor on durable single targets', controller.settings.fire.useMeteorSingleTarget, (value) => { controller.settings.fire.useMeteorSingleTarget = value; persist(); }), checkbox("Dragon's Breath in PvE packs", controller.settings.fire.useDragonsBreathPve, (value) => { controller.settings.fire.useDragonsBreathPve = value; persist(); }), range('Flamestrike enemies', controller.settings.fire.flamestrikeEnemyCount, 2, 8, String, (value) => { controller.settings.fire.flamestrikeEnemyCount = value; persist(); }), range('Meteor enemies', controller.settings.fire.meteorEnemyCount, 2, 8, String, (value) => { controller.settings.fire.meteorEnemyCount = value; persist(); }), range("Dragon's Breath enemies", controller.settings.fire.dragonsBreathEnemyCount, 2, 8, String, (value) => { controller.settings.fire.dragonsBreathEnemyCount = value; persist(); }), range('Conserve Fire mana', controller.settings.fire.conserveManaPct * 100, 0, 80, percent, (value) => { controller.settings.fire.conserveManaPct = value / 100; persist(); }), range('Stop Fire damage', controller.settings.fire.stopDamageManaPct * 100, 0, 50, percent, (value) => { controller.settings.fire.stopDamageManaPct = value / 100; persist(); }), range('Fire Aetherwell', controller.settings.fire.aetherwellManaPct * 100, 5, 80, percent, (value) => { controller.settings.fire.aetherwellManaPct = value / 100; persist(); }), range('Blazing Barrier below', controller.settings.fire.barrierHpPct * 100, 20, 100, percent, (value) => { controller.settings.fire.barrierHpPct = value / 100; persist(); }), range('Cold Coffin below', controller.settings.fire.iceBlockHpPct * 100, 10, 60, percent, (value) => { controller.settings.fire.iceBlockHpPct = value / 100; persist(); }));
