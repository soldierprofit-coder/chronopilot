...
function desiredSurgeCharges(group, contextMode, profile, settings, currentManaPct) {
    const maximum = clampSurgeCharges(contextMode === 'pvp'
        ? settings.pvp.maxSurgeCharges
        : settings.thresholds.maxSurgeCharges);
    const minimum = Math.min(maximum, clampSurgeCharges(contextMode === 'pvp'
...
function abilityReady(observation, settings, abilityId) {
    ...
    return (settings.abilities[abilityId] &&
...
function ownEchoTargets(observation) {
    return observation.party.filter((member) => (observation.individualEcho?.targetId === member.id ||
        member.auras.some((aura) => aura.kind === 'temporal_echo' &&
...
function smartGroupDamageHealingWindow(observation, settings, contextMode, profile, lowest) {
    ...
    return {
        active: settings.thresholds.smartPerfectMoment &&
            contextMode !== 'solo' &&
            safeToChannel &&
...
function assignedTank(observation) {
    ...
    const underPressure = living
        .map((member) => ({
        member,
        attackers: observation.enemies.filter((enemy) => enemy.inCombat && !enemy.dead && enemy.targetId === member.id).length,
    }))
        .sort((a, b) => b.attackers - a.attackers ||
        Number(b.member.hasAggro) - Number(a.member.hasAggro) ||
        a.member.id - b.member.id)[0];
    if (underPressure && (underPressure.attackers > 0 || underPressure.member.hasAggro)) {
        return underPressure.member;
    }
    const frontliner = living.find((member) => member.id !== observation.player.id && FRONTLINE_CLASSES.has(member.playerClass ?? ''));
    if (frontliner)
        return frontliner;
    return (living.find((member) => member.id === observation.partyLeaderId) ??
...
function adaptiveEchoTarget(observation, group, tank, profile) {
    ...
    if (!tankSafe)
        return tank;
    const isPressured = (member) => member.hasAggro || observation.enemies.some((enemy) => enemy.inCombat && !enemy.dead && enemy.targetId === member.id);
    ...
    const endangered = group
        .filter((member) => member.id !== tank.id &&
        distance(observation.player, member) <= FRIENDLY_RANGE &&
        isEndangered(member))
        .sort((a, b) => effectiveHpPct(a) - effectiveHpPct(b) || a.id - b.id);
    if (endangered.length === 0)
        return tank;
    const current = endangered.find((member) => member.id === observation.individualEcho?.targetId);
    const critical = endangered.find((member) => effectiveHpPct(member) < profile.emergencyHpPct);
    ...
    if (current && (!critical || effectiveHpPct(current) < profile.emergencyHpPct))
        return current;
    return critical ?? endangered[0] ?? tank;
}
export function resolveContextMode(observation, settings) {
    if (settings.mode !== 'auto')
        return settings.mode;
    if (observation.pvp)
        return 'pvp';
    if (observation.raid || observation.party.length > 5)
        return 'raid';
    if (observation.party.length > 1)
        return 'party';
    return 'solo';
}
...
function selectedEnemy(observation, settings, contextMode) {
    ...
    if (settings.targeting.enemyMode === 'closest-engaged')
        return engaged[0] ?? current ?? remembered;
    ...
    const attackable = (mayAcquire ? observation.enemies.filter((enemy) => valid(enemy)) : engaged).sort((a, b) => distance(observation.player, a) - distance(observation.player, b) || a.id - b.id);
    if (settings.targeting.enemyMode === 'lowest-hp') {
        return ([...attackable].sort((a, b) => a.hp / Math.max(1, a.maxHp) - b.hp / Math.max(1, b.maxHp) || a.id - b.id)[0] ?? current ?? remembered);
    }
    if (settings.targeting.enemyMode === 'closest-in-range')
        return attackable[0] ?? current ?? remembered;
    return engaged[0] ?? attackable[0] ?? current ?? remembered;
}
export function decideChronomancy(observation, settings) {
    ...
    if (playerInStasis)
        return wait('Ice Block is active; hold until the immunity ends or you cancel it.');
    if (pvpMode &&
        gcdReady &&
        ...
        return cast('ice_block', 1, observation.controlled || observation.silenced
    ...
    if (player.castingAbility || player.channeling)
        return wait('A cast or channel is already active.');
    ...
    const smartDamageHealing = smartGroupDamageHealingWindow(observation, settings, contextMode, profile, lowest);
    const continuePerfectMomentHealing = settings.thresholds.smartPerfectMoment &&
        contextMode !== 'solo' &&
        perfectMomentActive &&
    ...
    if (contextMode !== 'solo' && hpPct(player) >= profile.emergencyHpPct) {
        const currentFriendly = group.find((member) => member.id === observation.currentTargetId && member.id !== player.id);
        if (settings.targeting.friendlyMode === 'current-friendly' &&
            currentFriendly &&
            hpPct(currentFriendly) < profile.mendHpPct) {
            directHealTarget = currentFriendly;
        }
        else if (settings.targeting.friendlyMode === 'tank-first' &&
            tank &&
            tank.id !== player.id &&
    ...
    if (pvpMode && gcdReady && abilityReady(observation, settings, 'temporal_hourglass')) {
        const rescue = [...group]
            .filter((member) => hpPct(member) <= settings.pvp.hourglassHpPct &&
        ...
        if (rescue) {
            return castAt('temporal_hourglass', 6, `Suspend ${rescue.name} in a protective Hourglass during critical PvP pressure.`, rescue.x, rescue.z, rescue.id);
        }
    }
    if (pvpMode && gcdReady && abilityReady(observation, settings, 'frost_nova')) {
        const nearbyAttackers = observation.enemies.filter((enemy) => enemy.hostile &&
            !enemy.dead &&
            !enemy.crowdControlled &&
            distance(player, enemy) <= 10);
        if (nearbyAttackers.length >= settings.pvp.frostNovaEnemyCount ||
            (nearbyAttackers.length > 0 && playerHp < settings.pvp.frostNovaHpPct)) {
            return cast('frost_nova', 7, `Root ${nearbyAttackers.length} nearby PvP attacker${nearbyAttackers.length === 1 ? '' : 's'} to relieve melee pressure.`);
        }
    }
    if (settings.modules.healing && gcdReady && abilityReady(observation, settings, 'temporal_rewind')) {
        const rewindTargets = group.filter((member) => member.maxHp > 0 && member.rewind / member.maxHp >= profile.rewindLossPct);
        if (rewindTargets.length >= profile.rewindCount) {
            return cast('temporal_rewind', 10, `${rewindTargets.length} allies have enough recoverable recent damage.`);
        }
    }
    if (settings.modules.healing && lowest && hpPct(lowest) < profile.emergencyHpPct) {
        if (settings.thresholds.smartPowerEcho &&
            ...
            return cast('power_echo', 30, `${lowest.name} is below the emergency threshold. Arm the repeated heal.`);
        }
        if (gcdReady && abilityReady(observation, settings, 'temporal_mend')) {
            return cast('temporal_mend', 31, `${lowest.name} is critically injured at ${Math.round(hpPct(lowest) * 100)}%.`, lowest.id);
        }
    }
    if (settings.modules.defensives &&
        ...
        const endangered = group.filter((member) => distance(player, member) <= MASS_BARRIER_RADIUS &&
            effectiveHpPct(member) < profile.massBarrierHpPct);
        if (endangered.length >= profile.massBarrierCount) {
            return cast('mass_barrier', 32, `${endangered.length} nearby allies are below the Mass Barrier threshold.`);
        }
    }
    ...
    if (pvpMode &&
        ...
        const secondary = observation.enemies
            .filter((enemy) => enemy.hostile &&
            !enemy.dead &&
            !enemy.crowdControlled &&
            enemy.id !== primary &&
            distance(player, enemy) <= FRIENDLY_RANGE)
            .sort((a, b) => Number(b.targetId === lowest.id) - Number(a.targetId === lowest.id) ||
            distance(player, a) - distance(player, b) ||
            a.id - b.id)[0];
        if (secondary) {
            return cast('polymorph', 37, `Control ${secondary.name} while ${lowest.name} is under pressure.`, secondary.id);
        }
    }
    if (settings.modules.healing &&
        contextMode !== 'solo' &&
        gcdReady &&
        ...
        if (cluster && cluster.count >= profile.cascadeCount) {
            return cast('temporal_cascade', 38, `${cluster.count} injured allies are clustered around ${cluster.center.name}; establish the group damage-healing window.`, cluster.center.id);
        }
    }
    if (settings.modules.healing &&
        ...
        if (desiredEchoTarget && distance(player, desiredEchoTarget) <= FRIENDLY_RANGE) {
            ...
            if (movingEcho || remaining < settings.thresholds.echoRefreshSeconds) {
                ...
                return cast('temporal_echo', 39, movingEcho
                    ? rescuingAlly
                    ...
                    : `Refresh adaptive Echo on ${desiredEchoTarget.name} with ${remaining.toFixed(1)} sec left.`, desiredEchoTarget.id);
            }
        }
    }
    if (pvpMode &&
        ...
        return cast('temporal_acceleration', 75, 'Accelerate the group during a stable active PvP pressure window.');
    }
    if (settings.modules.healing &&
        directHealTarget &&
        ...
        return cast('temporal_mend', 40, `${directHealTarget.name} is below the Temporal Mend threshold.`, directHealTarget.id);
    }
    if (settings.modules.defensives &&
        gcdReady &&
        ...
        const barrierTarget = group
            .filter((member) => (member.hasAggro || observation.enemies.some((enemy) => enemy.targetId === member.id)) &&
        ...
        if (barrierTarget) {
            return cast('temporal_barrier', 50, `${barrierTarget.name} is injured and still has aggro.`, barrierTarget.id);
        }
    }
    if (settings.safety.buffOutOfCombat &&
        ...
        return cast('arcane_intellect', 65, 'Apply Aether Insight before combat because the party buff is missing, expiring, or the roster changed.');
    }
    if (settings.modules.healing &&
        settings.targeting.keepEchoOnTank &&
        tank &&
        ...
        if (remaining < settings.thresholds.echoRefreshSeconds) {
            return cast('temporal_echo', 70, remaining > 0 ? `Refresh the tank Echo with ${remaining.toFixed(1)} sec left.` : 'Maintain the individual Echo on the assigned tank.', tank.id);
        }
    }
    ...
    if (!settings.modules.damageToHeal)
        return wait('Damage-to-heal is disabled.');
    if (contextMode !== 'solo' &&
        !player.inCombat &&
    ...
    if (!enemy)
        return wait('No valid enemy is available inside the configured range.');
    if (observation.currentTargetId !== enemy.id) {
        const source = settings.targeting.enemyMode === 'assist-member-target'
    ...
    const nearbyEngaged = observation.enemies.filter((candidate) => candidate.inCombat &&
        candidate.hostile &&
        !candidate.dead &&
        !candidate.crowdControlled &&
        distance(player, candidate) <= 10).length;
    if (gcdReady &&
        !conservingMana &&
        nearbyEngaged >= settings.thresholds.aoeEnemyCount &&
        abilityReady(observation, settings, 'arcane_explosion')) {
        return cast('arcane_explosion', 91, `${nearbyEngaged} engaged enemies are inside Aetherburst range.`);
    }
    if (player.inCombat &&
        ...
        return cast('perfect_moment', 92, contextMode === 'solo'
    ...
    if (gcdReady &&
        ...
        return cast('arcane_missiles', 93, perfectMomentActive
    ...
    if (gcdReady && abilityReady(observation, settings, 'arcane_surge')) {
        ...
        if (player.mana >= expectedCost) {
            return cast('arcane_surge', 94, surgeTarget.smart
